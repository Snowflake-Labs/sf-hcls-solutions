...
description: >
  ...
  Usage: /sf-hcls-solutions:clinical-quality-agent install
         /sf-hcls-solutions:clinical-quality-agent teardown
         /sf-hcls-solutions:clinical-quality-agent (defaults to install)
...
1. Clone or locate the sf-hcls-solutions repository:
   ```bash
   git clone https://github.com/Snowflake-Labs/sf-hcls-solutions.git /tmp/sf-hcls-solutions
   ```
   Or check `~/project/sf-hcls-solutions/`, current directory.
...
```
Usage:
  /sf-hcls-solutions:clinical-quality-agent install    — Install the solution
  /sf-hcls-solutions:clinical-quality-agent teardown   — Remove the solution
```
